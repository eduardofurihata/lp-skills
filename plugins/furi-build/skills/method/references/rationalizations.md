...
| Frase | Realidade |
|-------|-----------|
| "Achei um bug lateral, abro card de follow-up" | Follow-up é débito com nome bonito. Balde B → ciclo `/method` completo AGORA, no Gate de Convergência. Card de follow-up é privilégio do **reviewer** (`/homolog` e `/prod`, via `plugins/furi-ship/skills/prod/references/findings.md`), nunca saída do dev. BLOQUEADO. |
