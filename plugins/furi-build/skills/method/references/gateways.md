...
> Racionalizações para pular Gateway → `rationalizations.md` categorias 1, 3 e 9; as das linhas obrigatórias vivem com o mecanismo e com a doutrina: `follow-ups.md`, `principles/SKILL.md` e `front/SKILL.md` § Racionalizações proibidas, e as de step nas categorias 11 e 12.
...
- **Princípios (SOLID · DRY · KISS · YAGNI · LoD · Motores):** ✅ aplicados — [1 linha: o que a lente deste step cobrou — ver o reference do step]
- **Refatoração (tudo por onde passou):** ✅ [N] elevados — [o que subiu] / nada a elevar — verifiquei [X] e já estava no nível #1
- **Design (tokens · atomicidade · composição · estados · a11y):** ✅ [o que a lente cobrou — ver o reference do step]   ← só com superfície visual
...
- **Princípios:** o que a **lente** do step cobrou (§ *Princípios neste step*, no reference do próprio step). "N/A" não existe: nada a corrigir → escreva o que você verificou e não encontrou. Violação achada → triagem A/B/C como qualquer achado. **Declarar "SOLID" significa os cinco** (SRP, OCP, LSP, ISP, DIP).
- **Refatoração:** o que a passada **elevou** no perímetro (`principles/SKILL.md` § Refatoração contínua — perímetro e regra do saldo). Nos **Steps 1-6** o perímetro é o **artefato** (doc consolidado, story separada, UC quebrado), não o código: a linha nunca é vazia. "Nada a elevar" exige dizer o que foi verificado.
- **Design:** só com superfície visual — derivada no Step 4a (`04-spec.md` § Escopo derivado, nunca declarado). Declare o que a lente do step cobrou (§ *Princípios neste step*, no reference do próprio step). Sem superfície visual: `❌ N/A — sem superfície visual, derivado do Step 4a` **uma vez**, no Gateway 4a→4b; o Step 4b não roda e os gateways seguintes herdam.

## Tabela de Critérios (TODOS obrigatórios por linha)

| Gateway | Critérios específicos |
|---------|----------------------|
| **Gate Check inicial** | docs/01-problem/, docs/02-user-stories/, docs/03-use-cases/, docs/04-spec/ (e docs/04-design/, se tem superfície visual) contêm doc cobrindo esta feature. Exibir visualmente no chat antes de qualquer código. |
...
| **2 → 3** | Stories cobrem todas as personas; formato "Como X, quero Y para Z"; **princípios:** SRP (1 story = 1 necessidade de 1 persona, sem "e também") · DRY (sem stories gêmeas) · YAGNI (toda story rastreia a uma persona do Step 1) · KISS (linguagem de usuário, sem solução técnica embutida) · Motor (stories da mesma capacidade apontam ao mesmo motor); **refatoração:** story empilhada separada |
| **3 → 4** | Use Cases derivados (ator × fluxo × estado); tabela de assinaturas única (sem duplicata); seção `## Verificação de Realidade` com cada passo do happy path mapeado a arquivo:linha OU 🔨 gap; **princípios:** SRP (1 UC = 1 combinação, sem agrupar) · YAGNI (todo UC rastreia a uma story) · KISS (fluxo em passos de usuário, sem código) · Motor (a tabela de assinaturas é o esboço do contrato); **refatoração:** UC agrupado quebrado, assinatura duplicada fundida; **design:** cada UC lista seus **estados de tela** (vazio, carregando, erro, sucesso, limite) |
...
| Nenhum TC passou por workaround? | Todo fix aplicado no ciclo respeita os princípios (`/principles`). TC que só passa violando SRP/DRY = **FAILED disfarçado**, não PASSED. Fix vai **para o motor**, nunca de remendo no chamador |
| UI: evidência por estado × breakpoint? | Feature com superfície visual: cada TC de UI tem evidência nos **estados** (vazio, carregando, erro, sucesso, limite) e nos **breakpoints do projeto**, não só o happy path em desktop (`/front`) |
...
- Mobile iOS + Android? ✅ SIM / N/A (escopo derivado do Step 4a confirma feature sem superfície mobile)
