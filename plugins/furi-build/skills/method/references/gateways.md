...
- **Princípios (SOLID · DRY · KISS · YAGNI · LoD · Motores):** ✅ aplicados — [1 linha: o que a lente deste step cobrou — ver `lentes.md`]
- **Refatoração (tudo por onde passou):** ✅ [N] elevados — [o que subiu] / nada a elevar — verifiquei [X] e já estava no nível #1
- **Design (tokens · atomicidade · composição · estados · a11y):** ✅ [o que a lente cobrou — ver `lentes.md`]   ← só com superfície visual
...
- **Princípios:** o que a **lente** do step cobrou (`lentes.md` § Princípios — lente por step). "N/A" não existe: nada a corrigir → escreva o que você verificou e não encontrou. Violação achada → triagem A/B/C como qualquer achado. **Declarar "SOLID" significa os cinco** (SRP, OCP, LSP, ISP, DIP).
- **Refatoração:** o que a passada **elevou** no perímetro (`principles/SKILL.md` § Refatoração contínua — perímetro e regra do saldo; `lentes.md` diz por que a linha nunca é vazia nos Steps 1-6). "Nada a elevar" exige dizer o que foi verificado.
- **Design:** só com superfície visual — quando e por quê em `lentes.md` § Design — a quem se aplica. Declare o que a lente do step cobrou (`lentes.md` § Design — lente por step). Sem superfície visual: `❌ N/A — sem superfície visual, derivado do Step 4a` **uma vez**, no Gateway 4a→4b; o Step 4b não roda e os gateways seguintes herdam.
