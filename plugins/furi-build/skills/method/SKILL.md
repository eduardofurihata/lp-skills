...
**Esta skill é FERRO.** Ativada, vale para toda a conversa, do Step 0 ao 12, com transição automática — nunca "posso prosseguir?"; única pausa: decisão irreversível que só o usuário julga. **Precisão > tokens**: "posso pular, é simples" É a violação.
...
- **Sem artefato `.md` = step não executado.** **Fecha seco** — achado fora do escopo vai ao ledger; o Step 11 o resolve com um ciclo `/method`.
...
Binário, publicado antes de transitar, bloqueante, atômico, universal. `## Gateway Check — Step N → N+1`, linhas — ausente = não publicado: artefato e critérios do step · **Princípios** (o que o `/principles` cobrou; SOLID = os cinco) · **Refatoração** (o que subiu no perímetro, ou "já no nível #1") · **Design** (o que o `/front` cobrou, ou `N/A — derivado do Step 4`) · **Follow-ups** (N, A/B/C) · **Veredicto** ✅ LIBERADO / ❌ BLOQUEADO — motivo.

Fora do protocolo só typo, refactor sem mudança de comportamento, config sem código de produto e pergunta; dúvida → Gate Check.
