...
**`/fast` NÃO é uma metodologia própria. `/fast` É o `/method`** — mesmas regras, mesmas seções (as `## Step N` de `method/SKILL.md`), mesmos gateways, mesma disciplina. **A ÚNICA diferença é ONDE ele para:** roda os Steps 1 a 8 e encerra após o Code Review (Step 8) limpo.
...
<HARD-GATE>
1. NÃO redefina nenhum step aqui. Para CADA step, abra a seção do step em `method/SKILL.md` (`grep -n '^## Step N'`) e siga ao pé da letra — INCLUSIVE o Step 5 (quantidade de TCs = nota de complexidade 1-10 derivada dos Steps 3-4, teto de 10; ver `method/SKILL.md` § Step 5). /fast NÃO tem regra de test case própria — nada de ISTQB/12 técnicas/escalas paralelas.
...
Siga a **Ordem de Operações do `/method`** (leia `method/SKILL.md` por arquivo — o `/fast` **não invoca** o `/method`: ele **é** o protocolo, lido das mesmas seções). A PRIMEIRA ação é **invocar o `/solve`** — via **Skill tool** (`furi-build:solve`; a forma curta `solve` também resolve), padrão de qualidade — **10x acima da referência #1 do mercado**. Chamada real, não "seguir de memória": sem a invocação, o passo não aconteceu. Depois: Inventário de Docs → Gate Check → TaskCreate → executar os steps em sequência, publicando o **Gateway Check** de cada um no chat. **A diferença é só parar no Step 8.**

| Step | Seção | /fast roda? |
|------|-----------|-------------|
| 1 — Problema | `method/SKILL.md` § Step 1 | ✅ |
| 2 — User Stories | `method/SKILL.md` § Step 2 | ✅ |
| 3 — Use Cases | `method/SKILL.md` § Step 3 | ✅ |
| 4 — Spec | `method/SKILL.md` § Step 4 | ✅ |
| 5 — Test Cases (**ESCREVE**, não executa) | `method/SKILL.md` § Step 5 | ✅ |
...
- `kanban/06-todo/<feature>.md` **PERMANECE** (criado no Step 6, não deletado) — é a fila de QA. Já contém a seção `## Test Cases (QA)` com um `- [ ]` por TC **e a seção `## Follow-ups`** (ledger), ambas semeadas no Step 6 conforme `method/SKILL.md` § Step 6.
...
- "Vou inventar uma regra de test case mais completa (ISTQB, 12 técnicas, 50-100 TCs)" → NÃO. Step 5 = `method/SKILL.md` § Step 5, `nº TCs == nota de complexidade`, teto de 10. /fast não tem regra própria.
- "Essa feature é complexa, mereço passar de 10 TCs" → NÃO. Teto é 10. Transbordou? Quebre a feature (ver `method/SKILL.md` § Step 5).
...
- "Vou fazer Step 8 mental, sem relatório" → NÃO. Step 8 = relatório formal em `kanban/08-code-review/` (ver `method/SKILL.md` § Step 8).
