...
| com alvo | **alvo** | Fluxo abaixo: os alvos, os pilares e os pesos **desta** régua, rodando o motor de § Fluxo do modo alvo — que é quem dita os passos, a fórmula da nota e o critério de parada. |
...
**Leia § Fluxo do modo alvo, abaixo, agora** — é o motor desta passada: os passos, a triagem A/B/C, a fórmula da nota, o critério de parada e o Output final moram lá, e ele é a fonte única deles. É o mesmo motor que o `/ui` roda. Sem essa leitura, o modo alvo não roda. Daqui sai o que é **desta** régua:
...
## Fluxo do modo alvo — o motor

> **Esta seção é a fonte única do fluxo de avaliação.** Toda skill que dá nota a um alvo e o corrige até passar roda ESTE fluxo: `/principles` (clareza e princípios de engenharia) e `/ui` (design). Cada uma traz só o que é dela: a tabela de alvos, os seis pilares, a tabela de pesos e o que cada passo exige naquele domínio. **Sem a leitura desta seção, o modo alvo não roda.**
...
| "Já conheço o fluxo, sigo de memória" | A leitura desta seção é o que faz o modo alvo rodar. Sem ela não há baldes, não há nota e não há critério de parada — sobra opinião. BLOQUEADO. |
...
As do fluxo estão em § Fluxo do modo alvo → Racionalizações proibidas do motor; estas são da doutrina.
