...
4. **Abrir ou atualizar a PR — como o time escolheu.** Só com `Abre PR: sim`. Cards vêm dos **commits**, nunca do nome da branch; nenhum → publicação sem card, dita. Título com todas as keys, na posição que o § Work manda; corpo em três camadas, no template do § Pull-request se houver: `## O que foi feito` (leigo, antes/depois — o **mesmo** texto vai ao Jira) · `## Cards` · `## Summary` e `## Solução` · `## Como testar` · `## DevOps` (migrations, env, deps — o que a configuração do ambiente consome). PR já aberta → título e descrição regenerados e um comentário com o que entrou; nunca uma segunda PR.
