...
1. **Descobrir.** `/setup` via **Skill tool, a cada invocação**. Objetivo: o card (**anexos** inclusos: o que o solicitante viu) ou a descrição; vazio = card ativo. Código: a branch do `/setup § Branch`, atualizada — o que reproduz é o que se corrige. Com card, mova-o para o status da etapa `/work`.
