---
name: repro
description: 'Use ONLY when the user explicitly invokes /repro (bare /repro = the objective is whatever the conversation is already about), or when another skill invokes `furi-ship:repro` via the Skill tool. NEVER activate on your own initiative. — reproduces the report on the front as the user, judged ≥ 90; the dev sees the bug before it fires and the fix BEFORE the commit.'
effort: max
...
- [ ] Julgar a reprodução noutro subagente (relato + registro + evidência → "é o problema?", 0–100); ≥ 90 avança, senão de novo, sem limite
- [ ] Parar um passo antes do trigger e mostrar ao dev: "👉 Clique em / Execute: [elemento exato]", o que vai acontecer, a evidência; esperar ele dizer que viu
