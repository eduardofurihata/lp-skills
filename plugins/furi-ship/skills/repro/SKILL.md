...
**Modificador**: o bug é reproduzido no front, como o usuário, antes de qualquer código; o dev o vê duas vezes: com o bug (parada 1) e sem (parada 2, § Human Check). Raciocine fundo; não codifica. **Arquivo da skill: `.claude/ship/repro.md`** — lido no início, criado na primeira vez, atualizado ao aprender: superfícies, usuários e dados de teste.

1. **Descobrir.** `/jira` e `/setup` via **Skill tool, a cada invocação**. Objetivo: o card (**anexos** inclusos: o que o solicitante viu) ou a descrição; vazio = card ativo. Código: a branch do `/setup § Branch`, atualizada — o que reproduz é o que se corrige. Com card, `/jira § Sincronizar card` (**trabalho começou**).
