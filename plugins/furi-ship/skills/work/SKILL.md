...
4. **Fechar o commit — um estado, não um passo.** Implementado · revisado a frio (bugs, edge cases, padrões do projeto, segurança, "faz o que o card pede" — relatório em `obra/09-code-review/`) · testado com evidência **na superfície onde o usuário vê**, 100% dos casos, screenshot por caso em `obra/10-run-test/` · sem pendência conhecida · documentado (artefato de `obra/07-todo/` para `obra/12-done/`, `tests: passed`, ledger seco) · **um** commit com a key do card ativo onde o `/setup § Commit` manda — mover primeiro, commitar por último; nunca cria nem troca branch. Reprodução nesta conversa → antes do commit, `repro/SKILL.md` § Human Check. ~3 passadas sem convergir → pare e diga o que resistiu.
