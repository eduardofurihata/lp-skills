...
> **gh → integração → branch.** Nunca trabalhar sobre integração stale; nunca assumir `dev`; nunca renomear a branch de um lote. Um clone de ontem já é stale.
...
| `branch acumula cards` | `$partida` é um **lote aberto** (§ 3) → `git checkout $partida && git merge <integração>` (o card entra nela); senão → `git checkout -b <nome>`, como em `branch por card` | a branch do lote |
...
## 5 — Manter a branch atualizada

`branch por card` e `branch acumula cards`: se `origin/<integração>` andar durante o trabalho, trazer pra branch (`git fetch origin && git merge origin/<integração>`, resolvendo conflitos) — o `/method` revê e testa o resultado integrado. Branch nunca fica pra trás da integração. É também o que roda quando quem chamou **retoma** um card (modo CONTINUE): já na branch do registro, sincroniza antes de seguir.
