...
2. **Inferir das pastas, citando a fonte.** `.secrets/` por **nomes** (os arquivos e as chaves das variáveis dos `.env`; chaves privadas, `id_*` e JSON de conta de serviço não se abrem; dumps e exports são artefatos, não credencial), o README do cofre, `.env.example`, `.mcp.json`, arquivos de plataforma (Vercel, Netlify, Railway, Fly), terraform, workflows (segredos do GitHub, runner), compose, schema do banco. Provedores pelos prefixos das variáveis — nem todo `AWS_*` é AWS. Repo Eduzz/Labzz: o recorte deste projeto e um ponteiro para o mapa da conta, nunca uma cópia.
