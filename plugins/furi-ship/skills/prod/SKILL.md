...
Entregue ao **`pipeline/SKILL.md` § reconcile**: diagnóstico da faixa publicado, estágios fechados na ordem — `branch` · `commit` (`work-cycle`) · `push`/`pr` (`pr-publish`) · `integrado` (`pr-cycle`) · `publicado@homolog` → `configurado@homolog` → `verificado@homolog` (`deploy-run` → `env-config` → `smoke`) · **«GATE»** · `promovido` (`promote`) · `publicado@prod` → `configurado@prod` (**prod e homolog** — o resync igualou as branches) → `verificado@prod` — re-diagnóstico a cada um. **Não reimplemente motor aqui.**
