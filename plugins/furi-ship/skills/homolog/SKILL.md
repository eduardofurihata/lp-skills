...
`pipeline/SKILL.md` § composicao primeiro. `PR number` ou `KEY-N` → **preferência de ordem** (aquele primeiro), não restrição do objetivo: o estado do ambiente continua sendo o alvo. `/repro` · `/card` → funde. `/prod` → vence o mais distante: delega e este alvo **não roda**. `/work` · `/pull-request` → perdem para este.

## Convenções (CONTRATO)

- **`<integração>` é a branch de integração; `homolog` é o AMBIENTE publicado a partir dela** — os dois **detectados** por `pipeline/SKILL.md` § deploy-context, passo 1 e gravados no `deploy.md § Ambientes`. `dev` é só o nosso padrão; `develop`/`staging`/`homologacao` funcionam igual. Neste doc, "o dev" sem backticks é a pessoa que escreveu o PR.
...
- **Board e estrutura do Jira:** `/jira`. **Convenções do time:** `/setup` — o `pr-publish` lê `Abre PR`; o `pr-cycle` lê `Aprovação` e `Merge`. **Contexto de deploy:** `deploy.md`, via `pipeline/SKILL.md` § deploy-context. **Onde vive cada segredo:** `infra.md` (`/infra`), lido pelo `env-config`.
...
4. NÃO mergeie sem code review limpo, nem com QA pendente, nem com ledger de follow-up `ABERTO` — regras em `pipeline/SKILL.md` § pr-cycle, integrais.
...
3. **`pipeline/SKILL.md` § deploy-context** — topologia, `<integração>`, ambientes e URLs (`deploy.md`).
...
5. **`pipeline/SKILL.md` § composicao** — o alvo efetivo.
...
Entregue ao **`pipeline/SKILL.md` § reconcile**: diagnóstico publicado, estágios fechados na ordem — `branch` · `commit` (`work-cycle`) · `push`/`pr` (`pr-publish`) se estiverem abertos; depois `integrado` (`pr-cycle`) → `publicado@homolog` (`deploy-run`) → `configurado@homolog` (`env-config`) → `verificado@homolog` (`smoke`) — re-diagnóstico a cada um, e para no `verificado@homolog`.

Os motores são as seções `§ <motor>` do `pipeline/SKILL.md`. **Não reimplemente nenhum aqui** — se uma regra do ciclo de PR ou do deploy precisar mudar, ela muda no motor, para os quatro alvos. Na borda, o único motor que invoca skill é o `env-config`, que chama o **`/infra`** (`furi-ship:infra`) via Skill tool quando falta o `infra.md` — nunca reproduzido de memória.
...
- "Todo projeto meu tem `dev`, assumo" → NÃO. `pipeline/SKILL.md` § deploy-context, passo 1 a **toda** invocação — o nome real pode ser `develop`, `staging`, e fica gravado no `deploy.md`.
...
- "Sei o que o `/infra` faz, rodo de cabeça" → NÃO. Mencionar não é invocar.
