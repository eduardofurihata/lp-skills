...
3. **Gate de QA.** Cards = título, `## Cards` e commits. Artefato em `obra/12-done/` com QA 100% → só review; QA ausente ou falha → review com front-test do `## Como testar`; artefato em `obra/07-todo/` → feche o commit na branch da PR (`Skill(skill: "work")`) ou rejeite; sem card → pergunte; ledger `ABERTO` → rejeite.
...
7. **Devolver.** Estágios, PRs, run, validação, cards, achados (nenhum card criado); próximo: `/prod`.
