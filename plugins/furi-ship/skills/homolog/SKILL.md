...
Integração: a branch que os PRs miram; sem PR, a usual em `origin` (várias → pergunte), senão a default. Igual à produção (§ Prod; senão `main`, `master`, a default) ⇒ branch única, sem homolog: sugira `/prod`, sem invocar. Valor de secret, nunca. Incompleto ou commit aberto na branch → `Skill(skill: "work")` ou rejeite; conserto pontual in-place, re-review; reimplementar, abordagem errada ou ~3 rodadas sem convergir → rejeição. Fora do escopo: ponta que o dev podia ver → rejeita; só o review vê → **A** bug (reprodução) · **B** furo (verbatim, ausência, consequência) · **C** melhoria, no review, nunca card; excedente separável → request-changes, entrelaçado → re-split. Aprovar: review na PR (sua → comentário); `Aprovação: <pessoa>` → espera o dela; branch atrás → atualiza, re-revisa, pusha; merge como o § Pull-request manda, branch apagada, comentário no card com a PR mergeada. Rejeitar: request-changes (o quê, por quê, o que muda), card de volta ao status da etapa `/work`.
...
- [ ] Atualizar § Homolog; devolver estágios, PRs, run, validação, cards, achados; próximo: `/prod`
