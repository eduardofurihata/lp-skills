...
Aprova ou devolve; produção é o `/prod`. Com outras skills no argumento: entenda o que cada uma pede e execute tudo combinado, numa passada só — nunca uma antes ou depois da outra. **Cada `- [ ]` é uma tarefa:** `TaskCreate` um por item, `TaskUpdate` fecha antes da próxima.
...
- [ ] Diagnosticar: `pr`, `integrado`, ambiente; tudo fechado → gap zero
- [ ] Garantir a PR: faltando → `Skill(skill: "pull-request")`; abertas → uma por vez
- [ ] Revisar o diff, sempre você: faz o que o card pede? Bugs, edge cases, padrões, segurança
...
- [ ] Publicar: push na `<integração>`; esperar o run verde
...
Integração igual à produção (§ Prod) ⇒ branch única: sugira `/prod`. Incompleto → `Skill(skill: "work")` ou rejeite; ~3 rodadas → rejeição. Fora do escopo → no review, nunca card. Run vermelho seu: conserte, teto ~3; fila ou runner offline: reporte. Secret: pedido, nunca inventado → `Skill(skill: "infra")`. Falhou: volta ao passo dono, o card não muda.
