...
| `/vac log [N]` | **log** | `… vac-hook.mjs log --tail N` e publique a saída inteira: bloqueios, bypasses, fallbacks, carimbos. `--kind pedido` mostra a triagem da entrada (`references/pedido.md`); `--kind note`, os sinais de cobertura (`references/cobertura.md`). Toda linha `bypass` ou `fallback` é dívida até virar caso. |
| `/vac caso [id\|last]` | **ciclo** | `… vac-hook.mjs caso <id\|last> [--kind note]` gera um sample em `scripts/samples/` a partir da entrada do log (dentro do lp-skills) ou imprime para colar. Falso positivo do hook → `expect: pass` + `class: falso-positivo-…`; falha do modelo → vira caso de `evals/vac-*`. Depois `pnpm test:vac`. |
...
| Cartão (§ O cartão) | todo prompt, todo subagente, após compactação | ≤ 1100 caracteres (~290 tokens) por injeção; mapas: uma linha por arquivo |
...
| Cobertura (`scripts/vac-cobertura.mjs`) | `Stop` e `Write`/`Edit` de artefato | ~0,1 ms: contagem contra contagem na mesma string, sem disco e sem transcript |
| Verificador | gate ou pedido (`/vac <alvo>`) | um subagente por verificação |
| Triagem do pedido (`scripts/vac-pedido.mjs`) | todo prompt do usuário | string + regex; só lê disco quando o prompt cita arquivo que não resolve direto. Hoje **só anota** — nada injetado, nada bloqueado |
