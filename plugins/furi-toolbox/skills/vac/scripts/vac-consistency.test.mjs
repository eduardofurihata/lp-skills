...
//   cartão ≤ 1100 caracteres (e ≤ 1250 com o sufixo de subagente)
...
test("taxonomia.md cobre toda regra que o código emite", () => {
  const code = read(path.join(HERE, "vac-rules.mjs")) + read(path.join(HERE, "vac-ledger.mjs")) + read(path.join(HERE, "vac-hook.mjs")) + read(path.join(HERE, "vac-pedido.mjs")) + read(path.join(HERE, "vac-cobertura.mjs"));
  const rules = new Set([...code.matchAll(/\b(?:it(?:em)?|nota)\(\s*"([\w-]+)"/g)].map((m) => m[1]));
...
test("cartão: ≤ 1100 caracteres; com sufixo de subagente ≤ 1250", () => {
  const skill = read(path.join(HERE, "..", "SKILL.md"));
  const card = skill.match(/<!--\s*vac:card\s*-->([\s\S]*?)<!--\s*\/vac:card\s*-->/)[1].trim();
  assert.ok(card.length <= 1100, `cartão com ${card.length} caracteres`);
  // o sufixo de subagente é injetado junto (vac-hook.mjs § cmdCard) — o teto real do prompt mais caro
  const sufixo = read(path.join(HERE, "vac-hook.mjs")).match(/"\\nVocê é um subagente:([^"]*)"/)[0].slice(1, -1);
  assert.ok(card.length + sufixo.length <= 1250, `cartão + sufixo de subagente com ${card.length + sufixo.length} caracteres`);
