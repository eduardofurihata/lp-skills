...
**Decisão:** as skills dependem **só** de `reconcile`; `reconcile` conhece os 7 motores; nenhum motor conhece outro nem a skill. As skills externas (`/jira-board`, `/pull-request`, `/card`) são chamadas **pelos motores**, na borda.
...
**Justificativa:** "às vezes tenho um bug e quero `/repro /card` para criar o card com os passos precisos; depois o dev escolhe `/repro /work` ou só `/work`". O mecanismo já existia (`Skill(skill, args)` + "superposição, nunca edição"); o que faltava era a regra de direção, porque aqui a ordem é livre.
