...
| D10 | Migração das skills próprias | `git mv skills/<slug> skills/personal/<slug>` (8 skills) | Preserva histórico | Boa prática git; UC-16 | Recriar do zero (perde histórico) |
...
skills/
├── personal/
│   ├── ask/ chat/ chat-out/ claude-modes/ commit/ method/ solve/ todo/   (git mv)
│   └── make-dev/                                                                     (do labzz)
└── eduzz/
