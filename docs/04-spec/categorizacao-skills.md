...
| D10 | Migração das skills próprias | `git mv skills/<slug> skills/personal/<slug>` (7 skills) | Preserva histórico | Boa prática git; UC-16 | Recriar do zero (perde histórico) |
...
skills/
├── personal/
│   ├── ask/ chat/ chat-out/ claude-modes/ commit/ method/ solve/   (git mv)
│   └── make-dev/                                                                     (do labzz)
└── eduzz/
