...
> Estende o Round 4 (não substitui nada). Motivação (2026-09-10): dentro do `furi-builder` conviviam o **workflow** (`method`, `solve`, `todo`, `proto`, `card`, `work`, `pull-request`, `homolog`, `prod`, `jira-board` — um grafo de `requires`/invocações que só faz sentido instalado junto) e **ferramentas avulsas** que ninguém requer e não requerem ninguém. O pedido: as avulsas num plugin próprio, com nome em inglês.
...
| 41 | **`/work` invoca o `/solve` na ativação** (bloco "Ordem de Operações ao Ativar", `requires` ganha `solve`, Red Flag) — mesmo padrão de `card`, `method`, `todo`, `proto` | Era a única skill do grafo que chegava ao `/solve` só por tabela (via `/method`); o padrão da casa é chamada real na ativação | — |
...
- Remover symlinks em `~/.claude/skills/*` cujo target casa `…/GitHub/lp-skills/skills/*` (14 links: afl, ask, card, chat, chat-out, claude-modes, commit, jira, make-dev, merge, method, notion-pull, notion-push, pr, solve, todo, work).
