...
> Estende o Round 4 (não substitui nada). Motivação (2026-09-10): dentro do `furi-builder` conviviam o **workflow** (`method`, `solve`, `proto`, `card`, `work`, `pull-request`, `homolog`, `prod`, `jira-board` — um grafo de `requires`/invocações que só faz sentido instalado junto) e **ferramentas avulsas** que ninguém requer e não requerem ninguém. O pedido: as avulsas num plugin próprio, com nome em inglês.
...
| 32 | **Critério de pertencimento à `toolbox`**: categoria é dona primeiro — skill de trabalho é `eduzz/` antes de qualquer critério. Entre as pessoais: sem `requires` **e** nenhuma outra skill a lista em `requires` ou a invoca via Skill tool. Hoje: `ask`, `chat`, `chat-out`, `claude-shortcuts`, `ctt`, `make-dev`, `save`, `sync`, `video-teams` (9) | Objetivo e verificável por grep; "sozinha" precisa de definição pra não virar gosto — e a dona vem antes, senão skill de trabalho sem deps cairia no pacote pessoal | "De trabalho" é o que fala o vocabulário da empresa (board, key, URL, conta, status), não o que se usa no trabalho: método genérico — sem citar empresa, skill, linguagem ou forja — é `toolbox`. `solve` fica em `personal/`: não tem `requires`, mas 3 skills dependem dele (`method`, `proto`, `card`) — é a base do grafo, não uma ferramenta solta. Menção como fronteira (`/prod` citando `/sync` numa red flag) não é dependência |
...
**Efeito na máquina de quem já tinha o `furi-builder`**: no próximo auto-update ele perde as 8 avulsas; `/plugin install furi-toolbox@lp-skills` traz as avulsas de volta. Não há caminho automático — plugin novo é instalação nova.

## Decisões (Round 6 — `furi-builder` vira `furi-build` + `furi-ship`)

> Estende o Round 5 (não substitui nada). Motivação (2026-09-10): o grafo de dependências das 9 skills de `personal/` (invocação via Skill tool, leitura de `references/`, handoff) tem um corte limpo — **build** (`solve`, `method`, `proto`: como se constrói) e **ship** (`jira-board`, `card`, `work`, `pull-request`, `homolog`, `prod` + os motores de `prod/references/`: como o trabalho entra e chega ao ar). Só 3 arestas cruzam o corte (`card → solve`, `work → method`, `pr-cycle → todo`), todas na direção ship → build. Build nunca depende de ship. O pedido: duas pastas, dois plugins, `personal` extinta.
...
| 41 | **`/work` invoca o `/solve` na ativação** (bloco "Ordem de Operações ao Ativar", `requires` ganha `solve`, Red Flag) — mesmo padrão de `card`, `method`, `proto` | Era a única skill do grafo que chegava ao `/solve` só por tabela (via `/method`); o padrão da casa é chamada real na ativação | — |
...
- Remover symlinks em `~/.claude/skills/*` cujo target casa `…/GitHub/lp-skills/skills/*` (13 links: afl, ask, card, chat, chat-out, claude-modes, commit, jira, make-dev, merge, method, notion-pull, notion-push, pr, solve, work).
