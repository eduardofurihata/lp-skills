...
**`furi-ship` e `eduzz-builder` são irmãos e não se conhecem.** Um é o fluxo pessoal, o outro é o trabalho: compartilham a base (`furi-build`), não o vocabulário. Nenhuma skill de um nomeia uma skill do outro — nem para dizer "não sou você": skill se distingue **afirmando o próprio contexto** (o `/work` diz *personal Atlassian*, o `/jira` diz *Eduzz*), nunca negando a outra. Negação cruzada acopla os dois pacotes e apodrece assim que um dos lados é renomeado. O `pnpm check` garante: citar o pacote irmão — em `requires`/`handoff`/`boundary` ou no texto — falha o CI.
