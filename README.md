...
/plugin install furi-build@lp-skills       # o método (/principles, /ui, /solve, /method, /fast, /todo, /proto, /blind)
/plugin install furi-ship@lp-skills        # a entrega (alvos: /work, /pull-request, /homolog, /prod · modificadores: /repro, /card · config: /setup, /jira, /infra) — puxa o furi-build
/plugin install furi-toolbox@lp-skills     # ferramentas avulsas (/brain, /ask, /chat, /save, /sync, /make-dev, /proof, /video-teams…)
```
...
`furi-ship` **puxa o `furi-build` junto** (dependência): o `/work` e o `/repro` rodam o `/method`, o `/card` e os motores usam o `/solve` e o `/todo` — então instalar o pacote de entrega traz também o método que ele precisa. A dependência vai só nessa direção: `build` não sabe o que é Jira, PR nem deploy. O `furi-toolbox` não puxa nem é puxado por ninguém: cada skill dele funciona sozinha.
...
Cada pasta em `plugins/` **é** a raiz de um pacote, e as skills dela moram em `plugins/<pacote>/skills/<slug>/`. A categoria de cada skill é derivada do pacote (`furi-build` → `build`), e o mapa categoria↔pacote sai do catálogo gerado — a LP não repete essa lista. O nome de invocação (`/homolog`) vem do `name` no frontmatter do `SKILL.md`; a dependência cruzada entre pacotes (ship → build) é derivada do `requires`.

**Critério de pacote:** a skill que não tem `requires` **e** de quem nenhuma outra skill depende ou invoca vai para `furi-toolbox` — funciona sozinha (o `/proof` e o `/brain` são os casos-limite: sem dependências, sem dependentes, sem citar outra skill e sem uma linha que cite empresa, linguagem ou forja — auditam e analisam qualquer repositório, então são toolbox). O `/brain` leva o critério ao extremo: onde precisaria da segunda opinião que o `/blind` dá, ele carrega o **próprio** bloco de sessão cega (`claude -p --safe-mode`) e os prompts de sistema em seções do próprio `SKILL.md` — é o preço de ser instalável sozinho, e significa uma cópia a mais do bloco, fora do `grep -rn 'env -u CLAUDECODE' plugins/furi-build/skills/` que o `/blind` documenta. As que entram no grafo se dividem pelo que tocam: **`furi-ship`** é o `/jira`, o `/setup`, o `/infra`, a `pipeline` e toda skill que os lista em `requires` — quem fala com board, GitHub ou ambiente. Dentro dele há **um pipeline só**: os quatro **alvos** (`/work`, `/pull-request`, `/homolog`, `/prod`) declaram até que estágio vão e entregam ao loop da `pipeline` (skill **interna**, `user-invocable: false` — só hospeda os motores; o `pnpm check` exige que toda interna seja alvo de algum `requires`), que diagnostica onde o trabalho está e fecha só o que falta; os dois **modificadores** (`/repro`, `/card`) rodam a própria parte e delegam ao alvo, em qualquer ordem digitada (`/repro /prod` = `/prod /repro`); `/setup`, `/jira` e `/infra` são invocadas no Step 0 de todas e também digitáveis, para configurar um projeto novo; **`furi-build`** é o resto do grafo (`/principles`, `/ui`, `/solve`, `/method`, `/fast`, `/todo`, `/proto` — e `/blind`, que o `/method` invoca no Step 8 e no Step 9, por isso não é toolbox) — constrói sem saber o que é Jira, PR ou deploy. A dependência só vai de ship para build, nunca o inverso: o `/method` cita `/homolog`, `/prod` e `/setup` como fronteira, não como `requires`.

**O conhecimento permanente de um projeto mora em `.claude/`, versionado no repositório-alvo** — quatro arquivos, um dono cada. Os três do processo de entrega ficam na pasta **`.claude/ship-setup/`**: `setup.md` (convenções do time: branch, commit, PR, Jira — `/setup`), `infra.md` (mapa da infra e onde vive cada segredo, nunca o valor — `/infra`) e `deploy.md` (processo de deploy — `/prod` via `deploy-context`). O quarto, `.claude/patterns.md` (padrões de código — `/method` Step 4), fica na raiz de `.claude/`: o `furi-build` não conhece o ship e não escreve numa pasta com esse nome. Não é a esteira por feature (`docs/01-problem/` … `kanban/`) nem a memória da máquina (`~/.claude/projects/`, onde só o board do Jira mora). Um `.md` solto em `.claude/` não é auto-carregado — cada arquivo é **lido sob demanda, por quem usa algo dele**, nunca via `CLAUDE.md`. Cada um tem uma variante **`<x>.local.md`** — mesmo formato, **nunca versionada** (como o `settings.local.json`) — para o repositório de um time que não usa este processo: o setup de uma pessoa não vai pro git dos outros, e o `/setup` pergunta uma vez qual dos dois é o caso; o do time vence quando os dois existem. Quem está em outro pacote lê por caminho e não cria: o `/method` aplica o § Commit do `setup.md`; escrever é do dono.
...
**Skill que depende de outra a invoca via Skill tool** no ponto de uso (`furi-build:<nome>` / `furi-ship:<nome>`) e a lista em `requires` — mencionar não é invocar. Hand-offs ("Próximo: /pull-request") e fronteiras ("isso é o /prod") ficam como menção: o próximo passo é decisão do usuário. Caminho de arquivo dentro de uma skill diz o escopo: `<slug>/SKILL.md` é do **mesmo pacote** (resolve no cache instalado) — as skills de `furi-build` e `furi-ship` são **mono-arquivo**, e o que era `references/x.md` virou seção `§ <Seção>` do próprio `SKILL.md` (inclusive o `/blind`: o comando da sessão cega e os três prompts de sistema moram em seções dele, e quem o usa copia o bloco de lá); `plugins/<pacote>/skills/<slug>/…` é de **outro pacote** e entra só como menção — o conteúdo chega invocando a skill dona; `docs/…`, `kanban/…`, `.claude/…`, `.github/…`, `.secrets/…` são do **projeto-alvo** — lidos por caminho no repositório onde a skill roda, e ler um deles não cria dependência entre pacotes (o validador conhece essas raízes em `TARGET_PROJECT_ROOTS`).
...
**Skill que roda em subagente** (`context: fork`) declara `effort` e **não declara `model`**. Sem `model:` o fork usa o modelo padrão de subagente (ou o da sessão), então acompanha a atualização de modelo em vez de ficar preso a um alias — o `/save` (`background: false`, a sessão espera o retorno inline) e o `/proof` (`background` omitido, a sessão segue e o relatório chega como notificação) são os exemplos. `model:` só entra quando a escolha for deliberada e para baixo (`haiku`, `sonnet`): nunca para fixar o modelo grande, que é o default e muda de nome.

**Skill que precisa de uma opinião que não herda nada** usa o `/blind`, não um subagente: `claude -p --safe-mode --tools ""` abre uma sessão sem a conversa, sem `CLAUDE.md`, sem memória, sem MCP, sem plugin e sem ferramenta (ou só `Read`/`Grep`/`Glob`, no modo `review`), com o login da sessão. Um fork herda o harness, o `CLAUDE.md` e o enquadramento de quem o chamou — tira o custo de contexto, não o viés. O `/method` usa o `/blind` onde o veredicto é julgamento do próprio trabalho: a revisão fria que fecha o Step 8 e o juiz pareado que fecha o TC de texto de IA no Step 9. Mesma regra de modelo: `--model` só para rebaixar.
