...
1. **Code review do diff** (calibre Step 8 do `/method`): `gh pr diff <n>` → revisar cada arquivo — bugs, edge cases, padrões do projeto (`docs/00-context/technical/patterns.md`), segurança, performance, código morto, "faz exatamente o que o card pede". Relatório em `kanban/08-code-review/<feature>.md` se ainda não houver.
   - **Princípios, um a um e por nome** (`skills/method/references/principios.md` — a mesma lista contra a qual o dev escreveu): **SRP** (responsabilidade única, camadas, >40 linhas) · **DRY** (duplicou o que já existe no projeto? conferir com grep, não de olho) · **KISS** · **YAGNI** (entrou abstração que nenhum UC pede?) · **Law of Demeter / acoplamento / direção de dependências**. Achado aqui é issue de review como qualquer outro: conserta in-place se for pontual, **rejeita** se limpar exigir reimplementar (passo 3).
