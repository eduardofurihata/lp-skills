...
fix(AV-36): [descrição concisa do que foi corrigido]
...
### 19.3 — Push
```bash
git push origin AV-36
```

### 19.4 — Criar PR
```bash
gh pr create --title "fix(AV-36): [título conciso]" --body "$(cat <<'EOF'
## O que foi feito
...
```
mcp__atlassian__jira_post → /rest/api/3/issue/AV-36/comments
```
...
```
mcp__atlassian__jira_post → /rest/api/3/issue/AV-36/transitions
```
Status: "Em revisão" | "Code Review" (conforme workflow do projeto).

### 19.6 — Mover card .md para done
```bash
mv docs/jira/todo/AV-36.md docs/jira/done/AV-36.md
```
...
- PR: [URL]
- Jira: Em revisão
- Card .md: docs/jira/done/AV-36.md
```
