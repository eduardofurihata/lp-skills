...
const PACKAGES = [
  {
    ...
    category: "build",
    description:
      "Skills de construção do Furihata — /solve, /principles, /front, /method e /proto: do problema ao commit local, com QA. Funciona sozinho, em qualquer repositório.",
  ...
  },
  {
    ...
    category: "ship",
    description:
      "Skills de entrega do Furihata — uma escada só: os alvos /work, /pull-request, /homolog e /prod levam o trabalho de onde estiver até o próprio estágio, cada um invocando o anterior quando falta algo atrás; os modificadores /repro e /card compõem com qualquer alvo, em qualquer ordem; /setup, /jira e /infra configuram o processo, o Jira e a infra do projeto (versionados em .claude/ship-setup/); a deploy, interna, põe um ambiente no ar e prova. Funciona sozinho, sem depender de outro pacote.",
